\echo 'Delete and recreate change_this_db_name db?'
\prompt 'Return for yes or control-C to cancel > ' foo

DROP DATABASE IF EXISTS change_this_db_name;
CREATE DATABASE change_this_db_name;
\connect change_this_db_name;

-- TODO:UPDATE ALL THIS TABLE SETUP


CREATE TABLE change_this_table_name (   
...
CREATE TABLE reservations (
    id integer PRIMARY KEY GENERATED ALWAYS AS IDENTITY,
    customer_id integer NOT NULL REFERENCES change_this_table_name,
...
COPY public.change_this_table_name (id, first_name, last_name, phone, notes) FROM stdin;
...
SELECT pg_catalog.setval('public.change_this_table_name_id_seq', 100, true);
...
DROP DATABASE IF EXISTS change_this_db_name_test;
CREATE DATABASE change_this_db_name_test;
\connect change_this_db_name_test;
